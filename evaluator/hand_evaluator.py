...
class HandEvaluator:
    ...
    class Two:
        ...
        def evaluate_percentile(hand, friend_cards=[]):
            """
            ...
            force_my_hand = HandEvaluator.Two.force_hand(hand)
            possible_opponent_hands = list(combinations(LookupTables.deck - set(hand) - set(friend_cards), len(hand)))

            hands_beaten = 0
            for h in possible_opponent_hands:
                possible_opponent_force = HandEvaluator.Two.force_hand(h)
                if force_my_hand > possible_opponent_force:
                    # you beat this hand
                    hands_beaten += 1
                elif force_my_hand == possible_opponent_force:
                    hands_beaten += 0.5
    ...
    def evaluate_hand(hand, board=[], friend_cards=[]):
        """
        ...
        if len(cards) == 2:
            return HandEvaluator.Two.evaluate_percentile(hand, friend_cards= friend_cards)
        elif len(cards) == 5:
        ...
        possible_opponent_hands = list(combinations(LookupTables.deck - set(cards) - set(friend_cards), len(hand)))
        hands_beaten = 0
        for h in possible_opponent_hands:
